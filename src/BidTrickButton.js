...
class BidTrickButton extends React.Component {

  render() {
    const {classes, buttonType, disabled, buttonKey, onClick, isCow, value} = this.props;
    ...
    if(buttonType === 'bid') {
      color = 'primary';
      if(isCow) {
        button = <Button className={classes.button} disabled={disabled} fab mini color={color} onClick={onClick} children={value}><CowIcon /></Button>
      }
      else {
        button = <Button className={classes.button} disabled={disabled} fab mini color={color} onClick={onClick} children={value}></Button>
      }
    }
    else {
      color = 'secondary';
...
export default withStyles(styles)(BidTrickButton);
